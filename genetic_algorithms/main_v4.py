...
class Map:
    ...
    def move(self, blob, direction):
        ...
        if direction == "sG":
            ...
            if y_diff < 0:
                the_map.move(blob, "mN")
            elif y_diff > 0:
                the_map.move(blob, "mS")
            elif x_diff < 0:
                the_map.move(blob, "mW")
            elif x_diff > 0:
                the_map.move(blob, "mE")
            else:
                direction = None
...
amount_of_neurons = 1000  # neurons make up a genome
...
amount_of_moves_per_generation = map_rows
for generation in range(amount_of_generations):
    ...
    for blob in blobs:
        ...
        """
        update_last_passing = True
        if blob.position[0] > the_map.cols - 3:
            # cleaning out the last passing genomes on the first run
            if update_last_passing:
                update_last_passing = not update_last_passing
                last_passing_neurons = []
            for genome in blob.genome:
                passing_blobs_neurons.append(genome)
                last_passing_neurons.append(genome)

    if len(passing_blobs_neurons) == 0:
        try:
            passing_blobs_neurons = last_passing_neurons
        except NameError:
