...
for i in range(10000):
    ...
    for x in range(batch_size):
        data = [np.random.uniform(-1, 1) for _ in range(batch_size)]
        training_data.append(data)
        targets.append([np.mean(data)])

    net.train(training_data, [targets], 0.1)

    """
    print("")
    print(training_data)
    print(targets)
    print(net.predict(training_data))
    """
